...
def analyze():
	if request.method == 'POST':
		...
		sample_data = [weigth,length1,length2,length3,height,width]
		...
		if model_choice == 'logitmodel':
		    logit_model = joblib.load('data/logit_model_fish.pkl')
		    result_prediction = logit_model.predict(ex1)
		elif model_choice == 'knnmodel':
			knn_model = joblib.load('data/knn_model_fish.pkl')
			result_prediction = knn_model.predict(ex1)
		elif model_choice == 'svmmodel':
			knn_model = joblib.load('data/svm_model_fish.pkl')
			result_prediction = knn_model.predict(ex1)

	return render_template('index.html', weight=weigth,
		length1=length1,
		length2=length2,
		length3=length3,
		height=height,
